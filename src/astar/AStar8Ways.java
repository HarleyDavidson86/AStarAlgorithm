 ...
 * the map. The distance between the nodes is always 1. 
 * Diagonal paths are implemented
 * @author Dominik Sust
 */
public class AStar8Ways implements AStar
{
    int mapHeight;
    int mapWidth;
    Node goalNode;
   ...
   /*
    ...
    private void expandNode(Node currentNode) throws Exception
    {
        ...
        // A node can have a maximum of 8 successor nodes
        //Nord
        if (currentNode.y>0)
        {
        ...
        //North-East
        if (currentNode.y>0 && currentNode.x<mapWidth-1)
        {
            Node nachfolger = getNodeAtXY( currentNode.x+1, currentNode.y-1);
            checkNachfolger(currentNode, nachfolger);
        }
        //North-West
        ...
        //South-East
        if (currentNode.y<mapHeight-1 && currentNode.x<mapWidth-1)
        {
            Node nachfolger = getNodeAtXY( currentNode.x+1, currentNode.y+1);
            checkNachfolger(currentNode, nachfolger);
        }
        //South-West
        if (currentNode.y<mapHeight-1 && currentNode.x>0)
        {
            Node nachfolger = getNodeAtXY( currentNode.x-1, currentNode.y+1);
            checkNachfolger(currentNode, nachfolger);
        }        
